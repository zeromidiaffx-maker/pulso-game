...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        if (!email || !password) {
            return res.json({ ok: false, erro: 'Email e senha obrigatórios' });
        }
        const hash = await bcrypt.hash(password, 10);
        const r = await pool.query(
            'INSERT INTO users (email, password_hash) VALUES ($1, $2) RETURNING id, email, wallet_balance',
            [email.toLowerCase(), hash]
        );
        res.json({ ok: true, msg: 'Conta criada!', user: r.rows[0] });
    } catch (e) {
        res.json({ ok: false, erro: e.message });
    }
});
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        if (!valid) return res.json({ ok: false, erro: 'Email/senha errados' });
        ...
        res.json({ 
            ...
            user: {
                ...
                wallet_balance: parseFloat(r.rows[0].wallet_balance)
            }
        });
    } catch (e) {
        res.json({ ok: false, erro: e.message });
...
app.post('/api/game/start', async (req, res) => {
    try {
        ...
        res.json({ ok: true, success: true, match_id: m.rows[0].id, bet_amount });
    } catch (e) {
        res.json({ ok: false, erro: e.message });
    }
});
...
app.post('/api/game/hit', async (req, res) => {
    try {
        ...
        if (!c.rows[0]) {
            return res.json({ ok: false, erro: 'Máximo de acertos atingido' });
        }
        
        const hit = Math.random() * 100 <= parseFloat(c.rows[0].zone_size_percentage);
        ...
        if (hit) {
            ...
            res.json({ 
                ...
                result: 'hit',
                hit_number: next,
                current_multiplier: parseFloat(c.rows[0].base_multiplier),
                potential_payout: (parseFloat(m.rows[0].bet_amount) * parseFloat(c.rows[0].base_multiplier)).toFixed(2)
            });
        } else {
            ...
            res.json({ 
                ...
                success: false,
                result: 'miss',
                loss: parseFloat(m.rows[0].bet_amount)
            });
        }
    } catch (e) {
        res.json({ ok: false, erro: e.message });
...
app.post('/api/game/cashout', async (req, res) => {
    try {
        ...
        const m = await pool.query('SELECT * FROM matches WHERE id = $1', [match_id]);
        const payout = parseFloat(m.rows[0].bet_amount) * parseFloat(m.rows[0].final_multiplier);
        const profit = payout - parseFloat(m.rows[0].bet_amount);
        ...
        const user = await pool.query('SELECT wallet_balance FROM users WHERE id = $1', [u.userId]);
        ...
        res.json({ 
            ...
            new_balance: parseFloat(user.rows[0].wallet_balance).toFixed(2)
        });
    } catch (e) {
        res.json({ ok: false, erro: e.message });
    }
